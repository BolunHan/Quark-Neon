...
class FactorPoolDummyMonitor(FactorMonitor):
    ...
    def __call__(self, market_data: MarketData, **kwargs):
        ...
        if timestamp < self.timestamp:
            self.cache_index = 0
            LOGGER.warning(f'Factor pool cache requires market data replayed in strict order for optimal performance. previous ts {self.timestamp}, new ts {timestamp}. cache idx is reset.')
    ...
    def factor_names(self, subscription: list[str]) -> list[str]:
        market_date = datetime.datetime.fromtimestamp(self.timestamp, tz=GlobalStatics.TIME_ZONE).date()
